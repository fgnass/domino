...
CustomElementRegistry.prototype.define = function(name, constructor, options) {
  if (!constructor) {
    var err = new TypeError('Provide name and constructor');
    throw err;
  }
  ...
  if (isInvalidCustomElementName(name)) {
    var err = new SyntaxError('Invalid name');
    err.code = 12;
    throw err;
  }

  if (this._registry[name]) {
    var err = new Error('Name already used');
  ...
  Object.keys(this._registry).forEach(function(_name) {
    if (constructor === _registry[_name]) {
      var err = new Error('Constructor already used');
      err.name = 'NOT_SUPPORTED_ERR';
      err.code = 9;
      throw err;
    }
  ...
  if (options && options.extends && (!isInvalidCustomElementName(options.extends) || forbiddenExtendsNames.indexOf(options.extends > -1))) {
    var err = new Error('Extend have to be build in type');
    err.name = 'NOT_SUPPORTED_ERR';
    err.code = 9;
    throw err;
  }
  ...
  ['connectedCallback',
    ...
    let type = typeof constructor.prototype[prop];
  ...
  this._clonesRegistry[name].prototype = Object.create(constructor.prototype);
  if (Array.isArray(constructor.observedAttributes)) {
    this._clonesRegistry[name].observedAttributes = constructor.observedAttributes;
  } else if (constructor.observedAttributes && constructor.observedAttributes[Symbol.iterator]) {
    this._clonesRegistry[name].observedAttributes = Array.from(constructor.observedAttributes[Symbol.iterator]());
  }

...
var htmlnames = require('./htmlnames');
...
Document.prototype = Object.create(Node.prototype, {
  ...
  createElement: { value: function(localName) {
    if (this.isHTML) {
        if (!htmlnames.isValidEltName(localName)) utils.InvalidCharacterError();
        localName = localName.toLowerCase();
    } else if (!xml.isValidName(localName)) utils.InvalidCharacterError();
    return html.createElement(this, localName, null);
  }},

  createElementNS: { value: function(namespace, qualifiedName) {
    // FIXME: Maybe the html5.TreeBuilder shouldn't be calling this.
    if (this.isHTML && namespace === NAMESPACE.HTML) {
      if (!htmlnames.isValidEltName(localName)) utils.InvalidCharacterError();
    } else if (!xml.isValidName(qualifiedName)) utils.InvalidCharacterError();

...
Element.prototype = Object.create(Node.prototype, {
  ...
  setAttribute: {value: function setAttribute(qname, value) {
    if (this.isHTML) {
      if (!htmlnames.isValidAttrName(qname)) utils.InvalidCharacterError();
      qname = qname.toLowerCase();
    } else if (!xml.isValidName(qname)) utils.InvalidCharacterError();
  ...
  setAttributeNS: { value: function setAttributeNS(ns, qname, value) {
    // FIXME: Again, maybe the html5.TreeBuilder shouldn't be calling this.
    if (this.isHTML && ns === null) {
      if (!htmlnames.isValidAttrName(qname)) utils.InvalidCharacterError();
    } else {

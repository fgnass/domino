...
function traverseChildren(tw, type) {
  ...
  while (node !== null) {
    result = tw._filter.acceptNode(node);
...
function traverseSiblings(tw, type) {
  ...
  while (true) {
    ...
    if (tw._filter.acceptNode(node) === NodeFilter.FILTER_ACCEPT) {
      return null;
    }
  }
}
...
function TreeWalker(root, whatToShow, filter) {
  ...
  // Getter reflects parameter
  tw.filter = filter;

  tw._filter = Object.create(NodeFilter.prototype);
   ...
   */
  tw._filter.acceptNode = function (node) {
...
TreeWalker.prototype = {
  ...
  parentNode: function () {
    ...
    while (node !== null && node !== this.root) {
      node = node.parentNode;
      if (node !== null && this._filter.acceptNode(node) === NodeFilter.FILTER_ACCEPT) {
  ...
  previousNode: function () {
    ...
    while (node !== this.root) {
      ...
      while (sibling !== null) {
        node = sibling;
        result = this._filter.acceptNode(node);
      ...
      node = node.parentNode;
      if (this._filter.acceptNode(node) === NodeFilter.FILTER_ACCEPT) {
        this.currentNode = node;
        return node;
      }
    }

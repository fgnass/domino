...
var $doc, $localName, $prefix;

exports.createElement = function(doc, localName, prefix) {
  var impl = doc._customElementRegistry.get(localName);
  var el;
  if (impl) {
    // I did not fount better solution.
    // We have to provide the $doc, $localName, $prefix to super constructor (which is HTMLElement or some derived class)
    // But we can't force the Custom Elements constructor to derived it by self
    // on other hand super have to be called during impl synchronously
    // so we can use the clause scope to deliver $doc, $localName, $prefix
    $doc = doc;
    $localName = localName;
    $prefix = prefix;
    el = new impl();
    ...
    impl = htmlNameToImpl[localName] || HTMLUnknownElement;
    el = new impl(doc, localName, prefix);
  }
  return el;
};
...
var HTMLElement = exports.HTMLElement = define({
  ...
  ctor: function HTMLElement(doc, localName, prefix) {
    // because custom elements will not deliver args, we will try to take them from clause scope
    Element.call(this, doc || $doc, localName || $localName, utils.NAMESPACE.HTML, prefix || $prefix);
  },
  props: {
    innerHTML: {
      get: function() {
      ...
      set: function(v) {
        var parser = this.ownerDocument.implementation.mozHTMLParser(
          ...
          this,
          {
            customElementsRegistry: this.ownerDocument._customElementRegistry
          });
        // For Custom Elements we have to prevent notify callbacks during parsing to technical document
        ce.preventNotify = true;
        ...
        // We need to notify old about remove
        ce.preventNotify = false;
        // Remove any existing children of this node
        while(target.hasChildNodes())
          target.removeChild(target.firstChild);
        // and we should notify new about remove and adopt
        ce.preventNotify = true;
        // Now copy newly parsed children from the root to this node
        target.doc.adoptNode(root);
        // but we have to notify new about connect to dom
        ce.preventNotify = false;
